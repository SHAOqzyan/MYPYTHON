...
class DBSCAN2D(object):
    ...
    maskBRange=[-5.25,5.25] #this region has MWISP CO observations,
    ...
    def sumEdgeByCon1(self, extendMask):  # 7 in total
        raw = extendMask[  1:-1, 1:-1]

        leftShiftY = extendMask[  0: -2, 1:-1]
        rightShiftY = extendMask[  2:, 1:-1]

        leftShiftX = extendMask[  1:-1, 0:-2]
        rightShiftX = extendMask[  1:-1, 2:]

        sumAll = raw + leftShiftY + rightShiftY + leftShiftX + rightShiftX
    ...
    def sumEdgeByCon2(self, extendMask):  # 27 in total
        ...
        for j in [-1, 0, 1]:
            for k in [-1, 0, 1]:
                ...
                sumAll = sumAll + extendMask[  j + 1:Ny + 1 + j, k + 1: Nx + 1 + k]

        return sumAll



    def maskByBrange(self,data,head,bRange=None):
        """
        set pixels in the bRange as 0
        :param bRange:
        :return:
        """

        if bRange is None:
            bRange=self.maskBRange
        wcsCO=  WCS(head, naxis=2)



        _,index0=wcsCO.wcs_world2pix( 180, bRange[0],0) #
        _,index1=wcsCO.wcs_world2pix( 180, bRange[1],0) #
        indexRage = [ index0,index1 ]

        indexRage=map(round,indexRage)
        indexRage=map(int,indexRage)

        index0,index1 = indexRage
        data[index0:index1+1,:] =0
    ...
    def getRMS2D(self,rmsData):
        """
        calculate the rms noise for each
        :param rmsData:
        :return:
        """
        # use the negative values

        negativeValues = rmsData [rmsData<=0]
        return  np.sqrt(np.mean(np.square(negativeValues)))


    def computeDBSCAN(  self ,doMask=False ):
        """
        ...
        COdata, COHead= doFITS.readFITS( self.rawCOFITS ) #cloud not be co data could be planck

        if doMask:
            self.maskByBrange(COdata,COHead)



        if len(COdata.shape)!=2:


            print "Please check your data, we only use 2D version of FITS"
            return


        Ny, Nx = COdata.shape

        print np.max(COdata) , np.min(COdata), "????????????????????"

        if self.averageRMS is None  :

            print "Calculating a meanRMS...."
            self.averageRMS = self.getRMS2D(COdata)


        print  "The average RMS of the fits is",self.averageRMS



        rmsCOData = COdata / self.averageRMS #essentially
        goodValues = rmsCOData >= self.cutoff_sigma #


        extendMask = np.zeros([  Ny + 2, Nx + 2], dtype=int)

        extendMask[ 1:-1, 1:-1] = goodValues  # [COdata>=minValue]=1

        s = generate_binary_structure(2, self.connectivity )
    ...
    def getLabelFITSName(self, doClean=False):
        ...
        if doClean:
            ...
            saveLabelFITSName= os.path.join(self.processPath,baseName[0:-5]+saveTag+"_2DClean.fits")

        else:
            saveLabelFITSName= os.path.join(self.processPath,baseName[0:-5]+saveTag+"2D.fits")
    ...
    def getCatFromLabelArray(self,doClean=True ):
        ...
        if self.averageRMS is not None and self.rawCOFITS is not None:
            ...
            self.averageRMS= self.getRMS2D( dataCO )
        ...
        wcsCloud = WCS(headCluster,naxis=2)
        ...
        for i in range(len(GoodIDs)):
            ...
            cloudIndex = self.getIndices2D( Y0, X0, clusterValue1D, newID)
            ...
            peakSimga =  self.minPeakSigma * self.averageRMS #only averageRMS is used for 2D version
            ...
            newRow["x_cen"], newRow["y_cen"] = wcsCloud.wcs_pix2world(Lcen, Bcen,   0)
    ...
    def produceCleanFITS(self):
        ...
        for eachDBRow in TB:
            ...
            cloudIndex = self.getIndices2D( Y0, X0, clusterValue1D, cloudID)
